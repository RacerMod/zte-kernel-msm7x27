...
2010-01-28   zt	    Update touchscreen driver.
                     register two interrupt and delet virtual key.   ZTE_TS_ZT_001
==========================================================================================*/
...
/* values from stock v9 pointercal */
...
#else
/* values from stock mooncake pointercal */
...
static int __devinit msm_ts_probe(struct platform_device *pdev)
{
	...
	for (i = 0; pdata->vkeys_x && (i < pdata->vkeys_x->num_keys); ++i)
		input_set_capability(ts->input_dev, EV_KEY, pdata->vkeys_x->keys[i].key);
	for (i = 0; pdata->vkeys_y && (i < pdata->vkeys_y->num_keys); ++i)
		input_set_capability(ts->input_dev, EV_KEY, pdata->vkeys_y->keys[i].key);
#endif
